...
class ExerciseOnDF(spark:SparkSession, util:CommonUtils) {
  ...
  def ColumnAndExpressionEx={
    ...
    totalProfitDf.where(col("Major_Genre").isNotNull  && lower(col("Major_Genre")) === "comedy")
  ...
  def sparkTypeAndDatasets={
    ...
    val numberDF = spark.read.format("csv").option("inferSchema",true)
      .option("header",true)
