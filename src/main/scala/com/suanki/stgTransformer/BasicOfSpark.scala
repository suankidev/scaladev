...
{"id":4,"name":"The Beatles","hometown":10,20,"test","year":1960}
    ...
    val failFastDf = spark.read.format("json").schema(dfSchema)
     ...
     * opions: json flag
     *
     * allowSinglQuotes -> true/false
     * compression -> uncomressed  //bazip2, gzip, snappy
     *
     * csv: options
     *
     * header
  ...
  def ColumnAndExpression():Unit={
    ...
    val carsDf = util.readDF(path=raw"src/main/resources/data/cars.json",
      filetype = "json"
