...
class JdbcFetch {
  ...
  /** val productDF = spark.read.format("jdbc") .option("url", "jdbc:oracle:thin:suanki/testpass@//localhost:1521/PDBORCL") .option("driver",
    * "oracle.jdbc.driver.OracleDriver") .option("user", "suanki") .option("password", "TestPass#123") .option("dbtable", "product").load()
...
//           d.close()
    ...
    val count_columns = """select count(PRODUCT_ID) as product_id, count(WAREHOUSE_ID) as WAREHOUSE_ID,
    ...
    val conn: Connection = DriverManager.getConnection("jdbc:oracle:thin:suanki/TestPass#123@localhost:1521/PDBORCL")
